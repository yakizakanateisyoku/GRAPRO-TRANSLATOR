...
def setup_logger(port):
    ...
    log_file = os.path.join("logs", "error_" + datetime.now().strftime("%Y%m%d") + "_" + str(port) + ".log")
    logging.basicConfig(
        level=logging.ERROR,
        format="%(asctime)s [PORT:%(name)s] %(levelname)s: %(message)s",
        handlers=[
            logging.FileHandler(log_file, encoding="utf-8"),
        ]
    )
    return logging.getLogger(str(port))
logger = None  # setup_logger(port)で初期化
...
if getattr(sys, 'frozen', False):
    _CONFIG_FILE = os.path.join(os.path.dirname(sys.executable), "config.json")

_DEFAULT_CONFIG = {
    "translate_engine": "deepl",
    "translate_url": "https://lt.f1234k.com/translate",
    "translate_api_key": "",
    "deepl_api_key": "",
        ...
        try:
            ...
            if changed:
                with open(_CONFIG_FILE, "w", encoding="utf-8") as f:
                    _json_cfg.dump(cfg, f, indent=2, ensure_ascii=False)
            return cfg
        except Exception:
            pass
    # デフォルトで新規作成
    with open(_CONFIG_FILE, "w", encoding="utf-8") as f:
        _json_cfg.dump(_DEFAULT_CONFIG, f, indent=2, ensure_ascii=False)
    return dict(_DEFAULT_CONFIG)

_cfg = _load_config()
TRANSLATE_ENGINE    = _cfg["translate_engine"]
LIBRETRANSLATE_URL = _cfg["translate_url"]
LIBRETRANSLATE_API_KEY = _cfg["translate_api_key"]
DEEPL_API_KEY      = _cfg["deepl_api_key"]
DEEPL_API_URL      = _cfg["deepl_api_url"]
AZURE_API_KEY      = _cfg["azure_api_key"]
AZURE_REGION       = _cfg["azure_region"]
AZURE_API_URL      = _cfg["azure_api_url"]
OVERLAY_PORT       = _cfg["overlay_port"]
MAX_MESSAGES       = _cfg["max_messages"]
MIN_CHARS          = _cfg["min_chars"]
...
# ===== オーバーレイ設定（サーバー側永続化） =====
_SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "overlay_settings.json")
if getattr(sys, 'frozen', False):
    _SETTINGS_FILE = os.path.join(os.path.dirname(sys.executable), "overlay_settings.json")
_DEFAULT_SETTINGS = {
    "bodyBg":        "transparent",
...
def _translate_deepl(text, source_lang):
    ...
    resp = requests.post(DEEPL_API_URL, headers=headers, data={
        "text": text,
        "source_lang": src,
        "target_lang": tgt,
    }, timeout=5)
...
def translate_text(text, source_lang):
    ...
    try:
        ...
        _stats.record_translation(OVERLAY_PORT, source_lang, True, _ms, len(text), len(result), original_text=text, translated_text=result)
        return result
    except Exception as e:
        _last_translate_error = str(e)
        _ms = (_time.time() - _t0) * 1000
        if logger: logger.error(f"translate_text[{TRANSLATE_ENGINE}] failed: {e} | text={text[:50]}")
        print(f"[翻訳エラー][{TRANSLATE_ENGINE}] {e}")
        _stats.record_translation(OVERLAY_PORT, source_lang, False, _ms, len(text), 0, str(e), original_text=text)
    return text
...
def chat_worker(video_id):
    ...
    while not stop_event.is_set() and retry < 5:
        try:
            ...
            if logger: logger.error(f"chat_worker crashed: {e} | video_id={video_id}")
            print(f"[チャットエラー] {e}")
...
<div id="panel">
  <h3>&#9881; オーバーレイ設定 <span style="font-size:11px;color:#888;font-weight:normal">v""" + VERSION + """</span></h3>
  <div class="sgroup">
...
def get_stats():
    ...
    port = int(flask_request.args.get('port', OVERLAY_PORT))
    return jsonify(_stats.get_summary(port=port, hours=hours))
...
def lt_url():
    ...
    if flask_request.method == 'POST':
        ...
        if new_url:
            ...
            return jsonify({"status": "ok", "url": LIBRETRANSLATE_URL})
        return jsonify({"status": "error", "message": "url is required"}), 400
    return jsonify({"url": LIBRETRANSLATE_URL})


@app.route('/retranslate')
def retranslate():
    """翻訳失敗メッセージ(translated==original)を再翻訳キューに投入"""
    count = 0
    with messages_lock:
        for msg in chat_messages:
            if msg.get("lang") == TARGET_LANG:
                continue
            if msg.get("translated") and msg["translated"] != msg["original"]:
                continue
            # 未翻訳 or 翻訳失敗 → キューに再投入
            translation_q.put({
                "author": msg["author"], "message": msg["original"],
                "lang": msg.get("lang", "en"),
                "imageUrl": msg.get("imageUrl", ""),
                "badgeUrl": msg.get("badgeUrl", ""),
                "isMember": msg.get("isMember", False),
                "isMod": msg.get("isMod", False),
...
if __name__ == '__main__':
    ...
    # ロガー起動
    logger = setup_logger(OVERLAY_PORT)

    # ワーカー起動
    start_workers()

    # 統計スナップショット開始
    _stats.start_snapshot_thread(
        OVERLAY_PORT,
        lambda: translation_q.qsize(),
        lambda: len(chat_messages),
    )
