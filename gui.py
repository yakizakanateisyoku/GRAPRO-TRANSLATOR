...
class App(ctk.CTk):
    ...
    def _build(self):
        ...
        self._platform_lbls = []
        for pname, pkey in [("YouTube","youtube"), ("Twitch","twitch"), ("ツイキャス","twitcasting"), ("SR","showroom")]:
        ...
        self._entry = ctk.CTkEntry(
            card, placeholder_text="YouTube / Twitch / ツイキャス / SR URL",
    ...
    def _get_vid(self, raw):
        """URL/IDの検証。YouTube, Twitch, ツイキャス, SHOWROOM, DEMO に対応"""
        raw = raw.strip()
    ...
    def _open_api_settings(self, _=None):
        ...
        current_engine = "grapro"
        try:
            r = SESSION.get(f"http://localhost:{PORT}/lt_url", timeout=0.5)
            if r.ok:
                d = r.json()
        ...
        for pname, pkey, placeholder in [
            ...
            ("SHOWROOM", "showroom", "https://showroom-live.com/r/ルーム名"),
        ]:
        ...
        _bm_port = 50001
        try:
            r = SESSION.get(f"http://localhost:{PORT}/bouyomi", timeout=0.5)
            if r.ok:
                d = r.json()
